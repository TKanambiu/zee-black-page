...
export function WhatsAppButton({ text, className = "" }: { text?: string; className?: string }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`group relative inline-flex items-center gap-3 overflow-hidden rounded-full bg-gradient-to-r from-[#25D366] via-[#22c55e] to-[#128C7E] px-7 py-3.5 font-semibold text-white shadow-[0_10px_30px_-8px_rgba(37,211,102,0.6)] ring-2 ring-white/30 transition hover:scale-[1.03] hover:shadow-[0_14px_40px_-8px_rgba(37,211,102,0.8)] ${className}`}
    >
      <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/25 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
      <span className="relative grid h-8 w-8 place-items-center rounded-full bg-white/20 ring-1 ring-white/40 backdrop-blur-sm">
        <svg viewBox="0 0 24 24" className="h-4.5 w-4.5 fill-white" width="18" height="18">
      ...
      <span className="relative tracking-wide">Order via WhatsApp</span>
      <span className="relative ml-1 rounded-full bg-white/25 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white ring-1 ring-white/40">Fast</span>
    </a>
  );
}
